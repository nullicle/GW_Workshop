...
class IBVP:
	...
	def run(self, tstart, tstop = float('inf'), thits = None):
		...
		while(computation_valid and t < tstop):
			...
			####################################################################
			# Chris' NaN Catcher START
			####################################################################
		   
			# Catch nan's and stop simulation even if nan appear on 
			# only one processor
			size = u.domain.mpi.comm.Get_size()
			rank = u.domain.mpi.comm.Get_rank()
			
			validity_this = np.isnan(np.sum(u.data))
			validity_all  = np.zeros(size, dtype=bool) # Initialize all to False

			validity_all[rank] = validity_this

			for i in range(0, size):
				validity_all[i] = u.domain.mpi.comm.bcast(validity_this, root=i)

			if validity_all.any():
				# print("NaNs detected!")
				computation_valid = False
				self.theSystem.systemstop = 1

			####################################################################
			# Chris' NaN Catcher STOP
			####################################################################
